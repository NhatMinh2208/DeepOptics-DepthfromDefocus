...
class RotationallySymmetricCamera(Camera):
    ...
    def psf_out_of_fov_energy(self, psf_size: int):
        ...
        return psf1d_out_of_fov.sum(), psf1d_out_of_fov.max()    
    #TODO: remove min_z
    def test_heightmap_diag(self, refractive_index):
        #R1 = self.focal_length / 2 
        R1 = self.focal_length * (refractive_index - 1)
        min_z = 1e10
    ...
    def test_heightmap_diag_mixed(self, refractive_index):
        #R1 = self.focal_length / 2 
        R1 = self.focal_length * (refractive_index - 1)
        min_z = 1e10
        Height = self.mask_diameter
    ...
    def test_heightmap_diag2(self, refractive_index):
        #R1 = self.focal_length / 2
        R1 = self.focal_length * (refractive_index - 1)
        min_z = 1e10
        Height = self.camera_pixel_pitch * self.image_size[0]
        half_mask_size = self.image_size[0] // 2
        heightmap1d = torch.zeros(half_mask_size)
        for i in range(half_mask_size): 
            x =  math.sqrt(2) * i * Height / self.image_size[0]
            z = math.sqrt(R1 * R1 - x * x)
            min_z = min(min_z, z)
            heightmap1d[i] = z
        heightmap1d = heightmap1d - min_z
...
class AsymmetricMaskRotationallySymmetricCamera(RotationallySymmetricCamera):
    def __init__(self, focal_depth: float, min_depth: float, max_depth: float, n_depths: int,
        ...
        super().__init__(focal_depth, min_depth, max_depth, n_depths, image_size, mask_size, focal_length,
                         mask_diameter, camera_pixel_pitch, wavelengths, mask_upsample_factor, diffraction_efficiency,
                           full_size, debug, requires_grad)
        #init_heightmap1d = torch.zeros(mask_size // 2 // mask_upsample_factor)  # 1D half size (radius)
        #init_heightmap1d = self.test_heightmap_diag(1.5626000901577761)
        init_heightmap1d = self.test_heightmap_diag(1.5)
        self.heightmap1d_ = torch.nn.Parameter(init_heightmap1d, requires_grad=requires_grad)
    ...
    def heightmap2(self):
        ...
        heightmap1d = self.test_heightmap_diag2()
        heightmap1d = heightmap1d.reshape(1, 1, -1)
        ...
        # heightmap = utils.helper.copy_quadruple(heightmap11).squeeze()
        heightmap = utils.helper.copy_quadruple(interp.linterp2(self.diag_grid, heightmap1d, self.diag_sampling, self.diag_ind)).squeeze()
        return heightmap
    ...
    def build_camera(self):
        ...
        self.diag_grid, self.diag_sampling = self.make_grid_from_diag(self.camera_pixel_pitch, self.image_size[0])
        self.diag_ind = self.find_index2(self.diag_grid, self.diag_sampling)
        self.side_grid, self.side_sampling = self.make_grid_from_side(self.camera_pixel_pitch, self.image_size[0])
        self.side_ind = self.find_index2(self.side_grid, self.side_sampling) 
    ...
    def make_grid_from_side(self, pitch, size):
        coord_y = pitch * (torch.arange(0, size // 2)).reshape(-1, 1)
        coord_x = pitch * (torch.arange(0, size // 2)).reshape(1, -1)
        coord_y = coord_y.double()
        coord_x = coord_x.double()
        ...
        r_grid = pitch * (torch.arange(0, size // 2 + 1, dtype=torch.double))
        #TOCHANGE
        r_grid = r_grid.to(self.device)
        return r_grid, r_sampling
    ...
    def mask2sensor_scale(self, value):
        ...
        '''
        #TODO: convert to sensor coordinate properly
    ...
    def mask2halfsensor_scale(self, value):
        '''
        Rescaling mask res to fit sensor res and make sure they have the same length (meter).
        '''
        #TODO: convert to sensor coordinate properly
        # sensor_coord_ = math.sqrt(2) * self.camera_pixel_pitch * (
        #         torch.arange(-1, max(self.image_size) // 2 + 1, dtype=torch.float) + 0.5).to(self.device)
        sensor_coord_ = 0.5 * self.camera_pixel_pitch * (
                 torch.arange(0, (max(self.image_size) * 2 ) // 2 + 1, dtype=torch.float) + 0.5).to(self.device)
    ...
    def h_half(self, h_size):
        '''
        The impulse response (h) of free-space propagation tell you how light is spread on the sensor.\n
        The h_size is currently choosen manually.
        '''
        pitch = self.camera_pixel_pitch * 0.5
        x = pitch * torch.arange( - (h_size // 2), h_size // 2 + 1).reshape(-1, 1)
        y = pitch * torch.arange( - (h_size // 2), h_size // 2 + 1).reshape(1, -1)
        distance_diff = x ** 2 + y ** 2
        distance_diff = distance_diff.unsqueeze(0).to(self.device)
        z = self.sensor_distance()
        k = (2 * math.pi) / self.wavelengths.reshape(-1, 1, 1).double()
    ...
    # def defocus_factor(self):
    #     device = self.device
    #     r = math.sqrt(2) * self.mask_pitch * torch.linspace(1, self.mask_size / 2, self.mask_size // 2, device=device).double()
    #     # compute pupil function
    #     wavelengths = self.wavelengths.reshape(-1, 1, 1).double()
    #     scene_distances = scene_distances.reshape(1, -1, 1).double()  # 1 x D x 1
    #     r = r.reshape(1, 1, -1)
    ...
    def defocus_factor_half(self, scene_distances):
        img_size = self.image_size[0] * 2
        pitch = self.camera_pixel_pitch * 0.5
        x = pitch * torch.arange( - ( img_size // 2),  img_size // 2).double().reshape(-1, 1)
        y = pitch * torch.arange( - ( img_size // 2),  img_size // 2).double().reshape(1, -1)
        r_squared = x ** 2 + y ** 2
        r_squared = r_squared.unsqueeze(0).to(self.device)
        z = scene_distances
        term = torch.sqrt(z.reshape(-1, 1, 1).to(self.device) ** 2 + r_squared)
    ...
    def psf1d(self, H, scene_distances, modulate_phase=torch.tensor(True)):
        ...
        # prop_amplitude = utils.helper.copy_quadruple(interp.linterp2(self.diag_grid, prop_amplitude, self.diag_sampling, self.diag_ind))

        # prop_phase = self.mask2sensor_scale(prop_phase)
        # prop_phase = utils.helper.copy_quadruple(interp.linterp2(self.diag_grid, prop_phase, self.diag_sampling, self.diag_ind))

        prop_phase = self.defocus_factor(self.scene_distances)
        prop_amplitude = torch.ones_like(prop_phase)
        #print(prop_phase.shape)
        if modulate_phase:
    ...
    def psf2d(self, H, scene_distances, modulate_phase=torch.tensor(True)):
        ...
        heightmap1d_ = self.mask2halfsensor_scale(heightmap1d_)
        rho_grid1, rho_sampling1 = self.make_grid_from_diag(self.camera_pixel_pitch * 0.5, self.image_size[0] * 2)
        rho_index = self.find_index2(rho_grid1, rho_sampling1)
        wavelengths = self.wavelengths.reshape(-1, 1, 1).double()
        ...
        self.dump_heightmap2d2 = utils.helper.copy_quadruple(interp.linterp2(rho_grid1, (utils.helper.heightmap_to_phase(self.test_heightmap_diag2(1.5).unsqueeze(0), wavelengths ,1.5)), rho_sampling1, rho_index))
        phasedelay_heightmap2d = self.dump_heightmap2d
        
        defocus_phase = self.defocus_factor_half(self.scene_distances)
        ...
        # TODO: change this to mask size
        defocus_amplitude = amplitude = self.circular_mask(self.image_size[0] * 2).unsqueeze(0).unsqueeze(1).to(self.device)  
        #
        ...
        #TODO: h _size
        in_camera_phase = self.h_half(1000)
    ...
    def _psf_at_camera_impl(self, H, rho_grid, rho_sampling, ind, size, scene_distances, modulate_phase):
        ...
        #return self.psf2d(None, None, modulate_phase).float()
        result = self.psf2d(None, None, modulate_phase).float()
        result2 = self.crop_last_two_dimensions(result,  self.image_size[0] * 2, self.image_size[1] * 2)
        result3 = F.interpolate(result2, size=(self.image_size[0], self.image_size[1]), mode='bilinear', align_corners=False)
        return result3
    ...
    def psf_out_of_fov_energy(self, psf_size: int):
        ...
        scene_distances = utils.helper.ips_to_metric(torch.linspace(0, 1, steps=self.n_depths, device=device),
                                        self.min_depth, self.max_depth)
        psf1d_diffracted = self.psf1d_full(scene_distances, torch.tensor(True))
        psf1d_diffracted = self.get_diagonal(psf1d_diffracted)
        diff_normalization_scaler = psf1d_diffracted.sum(dim=(-1), keepdim=True)
        # Normalize PSF based on the cropped PSF
        # print(psf1d_diffracted.shape)
        # print(diff_normalization_scaler.squeeze(-1).shape)
        psf1d_diffracted = psf1d_diffracted / diff_normalization_scaler
        edge = psf_size / 2 * self.camera_pixel_pitch / (
                self.wavelengths.reshape(-1, 1, 1) * self.sensor_distance())
        # print(edge.shape)
        # print(self.rho_grid_full.unsqueeze(1).shape)
        rho_grid = math.sqrt(2) * self.camera_pixel_pitch * (torch.arange(0, self.full_size[0] // 2, dtype=torch.double)).to(self.device) 
        rho_grid = rho_grid.reshape(1, 1, -1) / (self.wavelengths.reshape(-1, 1, 1).to(self.device) * self.sensor_distance()).to(self.device)
        psf1d_out_of_fov = psf1d_diffracted * (rho_grid.unsqueeze(1) > edge).float()
        return psf1d_out_of_fov.sum(), psf1d_out_of_fov.max()

    def psf1d_full(self, scene_distances, modulate_phase=torch.tensor(True)):
        #return self.psf1d(self.H_full, scene_distances, modulate_phase=modulate_phase)
        result = self.psf2d(None, None, modulate_phase).float()
        return self.crop_last_two_dimensions(result,  self.full_size[0], self.full_size[1])
    ...
    def get_psf(self):
        ...
        if is_training:
            ...
            scene_distances = utils.helper.ips_to_metric(torch.linspace(0, 1, steps=self.n_depths, device=device),
                                            self.min_depth, self.max_depth)
        return self.psf2d(self, scene_distances, torch.tensor(True))
    ...
    def dump_depth_psf(self, is_training = False):
        ...
        psf = self.psf2d(None, None)
        #TODO: normalize
        # psf_scaler = psf.sum(dim=(-1, -2), keepdim=True)
        # psf = psf / psf_scaler
        # print("normalize")
        print(torch.max(psf))
        print(torch.min(psf))
    ...
    def circular_mask(self, shape):
        ...
        x_ld = torch.flip(mask, dims=(-2,))
        x_d = torch.cat([x_ld, mask], dim=-2)
    ...
    def get_diagonal(self, tensor):
        ...
        start_row = 0
        start_col = n // 2
        # Get the diagonal of the upper right quarter
        diagonal_upper_right = torch.diagonal(tensor[..., start_row:n // 2, 0:start_col], offset=0, dim1=-2, dim2=-1)
        return torch.flip(diagonal_upper_right, dims = [-1])
    ...
    def __init__(self, focal_depth: float, min_depth: float, max_depth: float, n_depths: int,
        ...
        super().__init__(focal_depth, min_depth, max_depth, n_depths, image_size, mask_size, focal_length,
                         mask_diameter, camera_pixel_pitch, wavelengths, mask_upsample_factor, diffraction_efficiency,
                           full_size, debug, requires_grad)
        #init_heightmap1d = torch.zeros(mask_size // 2 // mask_upsample_factor)  # 1D half size (radius)
        init_heightmap1d = self.test_heightmap_diag_mixed(1.5626000901577761)
        #init_heightmap1d = self.test_heightmap_diag(1.5)
        self.heightmap1d_ = torch.nn.Parameter(init_heightmap1d, requires_grad=requires_grad)
