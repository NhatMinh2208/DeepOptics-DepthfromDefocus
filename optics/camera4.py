...
class MixedCamera(RotationallySymmetricCamera):
    ...
    def build_camera(self):
        ...
        self.rho_grid2, self.rho_sampling2 = self.rho_sampling_()
        self.rho_index2 = self.find_index2(self.rho_grid2, self.rho_sampling2)
        # self.ind2 = self.rho_index2
    def find_index(self, a, v):
        a = a.squeeze(1).cpu().numpy()
        v = v.cpu().numpy()
        index = np.stack([np.searchsorted(a[i, :], v[i], side='left') - 1 for i in range(a.shape[0])], axis=0)
    ...
    def rho_sampling_(self):
        coord_y = self.camera_pixel_pitch * (torch.arange(0, self.image_size[0] // 2)).reshape(-1, 1)
        coord_x = self.camera_pixel_pitch * (torch.arange(0, self.image_size[1] // 2)).reshape(1, -1)
        coord_y = coord_y.double()
        coord_x = coord_x.double()
        # (mask_size / 2) x (mask_size / 2)
        r_sampling = torch.sqrt(coord_y ** 2 + coord_x ** 2)
        ...
        r_grid = math.sqrt(2) * self.camera_pixel_pitch * (torch.arange(0, max(self.image_size) // 2 + 1, dtype=torch.double))
        #TOCHANGE
        r_grid = r_grid.to(self.device)
        return r_grid, r_sampling
    ...
    def _psf_at_camera_impl(self, H, rho_grid, rho_sampling, ind, size, scene_distances, modulate_phase):
        ...
        #                                       wavelengths,
        #                                       1.5)
        return utils.helper.copy_quadruple(interp.linterp2(self.rho_grid2, psf1d, self.rho_sampling2, self.rho_index2))
