...
class RotationallySymmetricCamera(Camera):
    ...
    def heightmap(self):
        ...
        r_grid = torch.arange(0, self.mask_size, dtype=torch.float)
        y_coord = torch.arange(0, self.mask_size // 2, dtype=torch.float).reshape(-1, 1) + 0.5
        x_coord = torch.arange(0, self.mask_size // 2, dtype=torch.float).reshape(1, -1) + 0.5
    ...
    def pointsource_inputfield1d(self, scene_distances):
        ...
        r = self.mask_pitch * torch.linspace(1, self.mask_size / 2, self.mask_size // 2, device=self.device).float()
        #
        # compute pupil function
        wavelengths = self.wavelengths.reshape(-1, 1, 1).float()
        scene_distances = scene_distances.reshape(1, -1, 1).float()  # 1 x D x 1
        r = r.reshape(1, 1, -1)
        ...
        if not math.isinf(self.focal_depth):
            #TOCHANGE
            focal_depth = torch.tensor(self.focal_depth, device=self.device).reshape(1, 1, 1).float()  # 1 x 1 x 1
    ...
    def psf1d(self, H, scene_distances, modulate_phase=torch.tensor(True)):
        """Perform all computations in float for better precision. Float computation fails."""
...
class AsymmetricMaskRotationallySymmetricCamera(RotationallySymmetricCamera):
    ...
    def radius_sampling(self):
        ...
        r_grid = math.sqrt(2) * self.mask_pitch * (
                torch.arange(1, self.mask_size // 2 + 1, dtype=torch.float) + 0.5)
        #TOCHANGE
    ...
    def _rho_sampling(self):
        ...
        r_grid = math.sqrt(2) * self.camera_pixel_pitch * (
                torch.arange(1, max(self.image_size) // 2 + 1, dtype=torch.float) - 0.5)
        #TOCHANGE
        r_grid = r_grid.to(self.device)
    ...
    def pre_sampling(self):
        ...
        r_grid = math.sqrt(2) * self.mask_pitch * (torch.arange(0, self.mask_size // 2 + 1, dtype=torch.float))
        #TOCHANGE
        r_grid = r_grid.to(self.device)
        return r_grid, r_sampling
    def pre_sampling_fullsize(self):
        ...
        r_grid = math.sqrt(2) * self.mask_pitch * (torch.arange(0, self.full_size[0] // 2 + 1, dtype=torch.float))
        #TOCHANGE
        r_grid = r_grid.to(self.device)
        return r_grid, r_sampling
    def precompute_H(self, image_size):
        ...
        rho_grid = math.sqrt(2) * self.camera_pixel_pitch * (
                torch.arange(-1,    image_size // 2 + 1, dtype=torch.float) + 0.5)
    ...
    #     x = self.mask_pitch * torch.arange( - (self.mask_size // 2), self.mask_size // 2).float().reshape(-1, 1)
    #     y = self.mask_pitch * torch.arange( - (self.mask_size // 2), self.mask_size // 2).float().reshape(1, -1)
    #     r_squared = x ** 2 + y ** 2
    #     r_squared = r_squared.unsqueeze(0).to(self.device)
    #     z = scene_distances
    #     term = torch.sqrt(z.reshape(-1, 1, 1) ** 2 + r_squared)
    #     k = (2 * math.pi) / self.wavelengths.reshape(-1, 1, 1, 1).float()
    ...
    def defocus_factor(self, scene_distances):
        x = self.mask_pitch * torch.arange( 0, self.mask_size // 2).float()
        y = self.mask_pitch * torch.arange( 0, self.mask_size // 2).float()
        r_squared = x ** 2 + y ** 2
        r_squared = r_squared.unsqueeze(0).to(self.device)
        z = scene_distances.to(self.device)
        term = torch.sqrt(z.reshape(-1, 1) ** 2 + r_squared)
        k = (2 * math.pi) / self.wavelengths.reshape(-1, 1, 1).float()
        result = k * term.unsqueeze(0)
        # print('result')
        # print(result.shape)
        return result    
    ...
    def psf1d(self, scene_distances, modulate_phase=torch.tensor(True)):
        ...
        """
        return the PSF2D\n
        note: Perform all computations in float for better precision. Float computation fails."""
        wavelengths = self.wavelengths.reshape(-1, 1, 1).float()

        phasedelay_heightmap2d = utils.helper.heightmap_to_phase(self.heightmap1d_.unsqueeze(0).to(self.device),  # add wavelength dim
                                              wavelengths,
                                              1.5)
        ...
        in_camera_phase_complex = in_camera_phase_complex / self.wavelengths.reshape(-1, 1)  # n_wl , x, y
        ...
        self.dump_spectrum_conv = torch.fft.fft(phase_complex) * torch.fft.fft(in_camera_phase_complex)

        conv = torch.fft.ifft(self.dump_spectrum_conv)  
        #TODUMP
    ...
    def _psf_at_camera_impl(self, scene_distances, modulate_phase):
        ...
        #psf1d = psf1d[..., :self.image_size[0], :self.image_size[1]]
        # crop from the center
        psf1d = utils.helper.copy_quadruple(interp.linterp2(self.rho_grid, (psf1d), self.rho_sampling, self.rho_index))
        uncropped_size = psf1d.shape
        uncropped_height = uncropped_size[-1]
        uncropped_width = uncropped_size[-2]
        left = (uncropped_width - self.image_size[0]) // 2
    ...
    def psf_out_of_fov_energy(self, psf_size: int):
        ...
        scene_distances = utils.helper.ips_to_metric(torch.linspace(0, 1, steps=self.n_depths, device=device),
                                        self.min_depth, self.max_depth)
        # psf1d_diffracted = self.psf1d_full(scene_distances, torch.tensor(True))
        psf1d_diffracted = self.psf1d(scene_distances, torch.tensor(True)).float()
        #psf1d_diffracted = self.psf_at_camera_full
        # Normalize PSF based on the cropped PSF
        ...
        if (self.debug):
            ...
            print( psf1d_diffracted / self.diff_normalization_scaler)
        #psf1d_diffracted = psf1d_diffracted / self.diff_normalization_scaler.squeeze(-1)
        psf1d_diffracted = psf1d_diffracted / psf1d_diffracted.sum(dim=(-1), keepdim=True)
        # edge = psf_size / 2 * self.camera_pixel_pitch / (
        #         self.wavelengths.reshape(-1, 1, 1) * self.sensor_distance())
        # psf1d_out_of_fov = psf1d_diffracted * (self.rho_grid_full.unsqueeze(1) > edge).float()

        #create a mask 
        mask = torch.ones(3, self.n_depths, 400 + (self.mask_size // 2)).to(self.device)
        uncropped_size = mask.shape
        uncropped_height = uncropped_size[-1]
        right = psf_size // 2
        mask[..., 0:right] = 0
    ...
    def to_sensor_phase(self, h_size):
        ...
        The h_size is currently choosen manually.
        '''
        x = self.mask_pitch * torch.arange( - (h_size // 2), h_size // 2 + 1)
        y = self.mask_pitch * torch.arange( - (h_size // 2), h_size // 2 + 1)
        distance_diff = x ** 2 + y ** 2
        distance_diff = distance_diff.unsqueeze(0).to(self.device)
        z = self.sensor_distance()
        k = (2 * math.pi) / self.wavelengths.reshape(-1, 1).float()
        return k * distance_diff / (2 * z)
...
class MixedCamera(RotationallySymmetricCamera):
    ...
    def precompute_H(self, image_size):
        ...
        # sqrt(2) is for finding the diagonal of FoV.
        rho_grid = math.sqrt(2) * self.camera_pixel_pitch * (
                torch.arange(-1, max(image_size) // 2 + 1, dtype=torch.float) + 0.5)
        ...
        r = self.mask_pitch * torch.linspace(1, self.mask_size / 2, self.mask_size // 2).float()
        r = r.reshape(1, -1, 1)
    ...
    def pointsource_inputfield1d(self, scene_distances):
        device = self.device
        r = self.mask_pitch * torch.linspace(1, self.mask_size / 2, self.mask_size // 2, device=device).float()
        # compute pupil function
        wavelengths = self.wavelengths.reshape(-1, 1, 1).float()
        scene_distances = scene_distances.reshape(1, -1, 1).float()  # 1 x D x 1
        r = r.reshape(1, 1, -1)
        ...
        phase = wave_number * (radius - scene_distances)  # n_wl x D x n_r
        if not math.isinf(self.focal_depth):
            focal_depth = torch.tensor(self.focal_depth, device=device).reshape(1, 1, 1).float()  # 1 x 1 x 1
            f_radius = torch.sqrt(focal_depth ** 2 + r ** 2)  # 1 x 1 x n_r
            phase -= wave_number * (f_radius - focal_depth)  # subtract focal_depth to roughly remove a piston
