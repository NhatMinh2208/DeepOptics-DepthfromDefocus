...
class AsymmetricMaskRotationallySymmetricCamera(RotationallySymmetricCamera):
    ...
    def mask2sensor_scale(self, value):
        ...
        sensor_coord_ = math.sqrt(2) * self.camera_pixel_pitch * (
                torch.arange(-1, max(self.image_size) // 2 + 1, dtype=torch.double) + 0.5).to(self.device)
    ...
    def h(self, h_size):
        ...
        '''
        r = self.mask_pitch * torch.linspace(1, h_size / 2, h_size // 2, device=self.device).double()
        r_square = r ** 2
        r_square = r_square.unsqueeze(0).to(self.device)
        z = self.sensor_distance()
        k = (2 * math.pi) / self.wavelengths.reshape(-1, 1).double()
        return k * r_square / (2 * z)
    ...
    def psf1d(self, H, scene_distances, modulate_phase=torch.tensor(True)):
        ...
        E_complex = torch.complex( torch.cos(phase),  torch.sin(phase))
        ...
        E_complex = self.__padding2size(E_complex, pad_size)
        h_complex = self.__padding2size(h_complex, pad_size)
        # print(E_complex.shape)
        # print(h_complex.shape)
        # print(torch.fft.fft(E_complex).shape)
        # print(torch.fft.fft(h_complex).shape)
        spectrum_mul = torch.fft.fft(E_complex) * torch.fft.fft(h_complex)
        conv = torch.fft.ifft(spectrum_mul)  

        #----
        val2 = conv.real 
        self.dump_conv = utils.helper.copy_quadruple(interp.linterp2(rho_grid1, val2, rho_sampling1, rho_index))
        #----
        return self.mask2sensor_scale(conv.real ** 2 + conv.imag ** 2)  # n_wl X D X n_r -> n_rho
    ...
    def __padding2size2(self, tensor, new_size):
        """
        Padding zeros to the end of a complex-valued tensor.
        """
        x_padding = (new_size - tensor.shape[-1]) 
